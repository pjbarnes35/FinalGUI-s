...
class InventoryManagementSystem:
    def __init__(self, root):
        ...
        self.notification_button = ttk.Button(root, text="View Notifications", command=self.open_notifications)
        self.notification_button.grid(row=2, column=0, padx=10, pady=5)
    ...
    def submit_outgoing_form(self):
        ...
        # Check if the quantity of sandstone is low and show a mock notification
        if rock_type == "Sandstone" and int(quantity) < 10:
            messagebox.showinfo("Low Inventory Notification", "The quantity of Sandstone is running low. Please replenish the inventory.")

    def open_notifications(self):
        notification_window = tk.Toplevel(self.root)
        notification_window.title("Notifications")

        notification_text = "Mock SMS Notification: The quantity of Sandstone is running low. Please replenish the inventory."
        ttk.Label(notification_window, text=notification_text).pack(padx=10, pady=10)
